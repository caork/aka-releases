...
Gitee 只提供相同的 Linux `base`/`packs` 双卷及 Gitee 自己的 `SHA256SUMS` 与 `latest.json`，不提供 Windows 二进制。普通 Windows 安装包和普通 Linux headless 服务包不属于未来正式发行。
