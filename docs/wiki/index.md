...
| 选择 | 适用场景 | 包含内容 |
| --- | --- | --- |
| Linux complete 包 | 隔离网络、受控网络，或需要在首轮索引就使用上游语义增强 | 单一 `aka-headless-complete-*.tar.gz`，内置 Java、Python、TypeScript/JavaScript、C/C++、Rust 五个已签名 `.aka-pack`；一次解压后运行 `./start.sh` 即可启动，不需要首次联网下载。 |
