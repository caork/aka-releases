...
AKA 的历史 legacy engine 含 MIT 派生代码。其显式 parity 回退窗口已经结束，相关实现与发行路径已删除；必要的 MIT 来源与许可说明仍须永久保留。源码或发行渠道的变化不撤回历史已公开内容，也不会取消该许可说明义务。

当前运行时只使用内置 Rust `aka-parse`，不提供 legacy parity 回退。分发者仍应保留所附的第三方 notices，不得移除或篡改。
